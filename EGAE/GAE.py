...
class EGAE_encoder(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        z_egae = self.gnn_3(z, adj, active=False)
        z_egae_adj = self.s(torch.mm(z_egae, z_egae.t()))
        return z_egae, z_egae_adj


class EGAE_decoder(nn.Module):

    def __init__(self, gae_n_dec_1, gae_n_dec_2, gae_n_dec_3, n_input):
        super(EGAE_decoder, self).__init__()
    ...
    def forward(self, z_egae, adj):
        z = self.gnn_4(z_egae, adj, active=True)
...
class EGAE(nn.Module):
    ...
    def __init__(self, gae_n_enc_1, gae_n_enc_2, gae_n_enc_3, gae_n_dec_1, gae_n_dec_2, gae_n_dec_3, n_input):
        ...
        self.encoder = EGAE_encoder(
            gae_n_enc_1=gae_n_enc_1,
            gae_n_enc_2=gae_n_enc_2,
            gae_n_enc_3=gae_n_enc_3,
            n_input=n_input)

        self.decoder = EGAE_decoder(
    ...
    def forward(self, x, adj):
        z_egae, z_egae_adj = self.encoder(x, adj)
        z_hat, z_hat_adj = self.decoder(z_egae, adj)
        adj_hat = z_egae_adj + z_hat_adj
        return z_egae, z_hat, adj_hat
